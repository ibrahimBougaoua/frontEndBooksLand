...
async function ifShowLater(id) {
  try {
    const response = await axios.get('http://127.0.0.1:5001/book/check/showlater/user/' + localStorage.getItem('email') +'/id/' + id);
    console.log(response);
    return response;
  } catch (error) {
    console.error(error);
...
state = { value: 0,visibleComponent: false,enable: false,book: [],related: [] };
...
const delete_show_late = () => {
  axios.get('http://127.0.0.1:5001/book/showlater/delete', {params : {_email:  localStorage.getItem('email'),  _id: this.props.match.params.id}})
  .then(function (response) {
    // setter
    //const token = localStorage.setItem('token', response.data.access_token)
    //const user = localStorage.setItem('user', response.data.user)
...
<div className="card mb-5 border-0 rounded-0 shadow-sm bg-light">
	<div className="row no-gutters">
        ...
        <p className="text-sm-left" style={{fontSize:"1vw"}}>{element}</p>
				<Link className="btn btn-sm btn-outline-info mr-2" to={"/account/author/" + element}>More</Link>
			</div>
		</div>
	</div>
</div>
