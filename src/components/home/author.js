...
async function authorBooks(author) {
    try {
...
state = { visibleComponent: false,checked: false,book: [] }
...
componentDidMount =()=>{    
  if(localStorage.getItem('email')){
	...
	authorBooks(this.props.match.params.author).then(response => {
	  this.setState({
      book: response.data
	  });
	});
...
const new_Abonnement = () => {
    axios.get('http://127.0.0.1:5001/book/abonnement/new', {params : {_email:  localStorage.getItem('email'),  _actor: this.props.match.params.actor}})
...
const books = this.state.book.map((element) => 
        ...
        
{
  this.state.visibleComponent
    ? 	<p className="card-text"><button type="submit" className="btn btn-sm btn-success disabled" onClick={new_Abonnement}>Abonner</button></p>
    : null
} 
