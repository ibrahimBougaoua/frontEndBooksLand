...
async function ifAbonner(author) {
  try {
    const response = await axios.get('http://127.0.0.1:5001/book/check/abonnement/user/' + localStorage.getItem('email') +'/author/' + author);
...
// handle button click of login form
async function authorsBooks(author) {
  try {
    const response = await axios.get('http://127.0.0.1:5001/book/abonnements/author/' + author);
    console.log(response);
    return response;
  } catch (error) {
    console.error(error);
...
state = { checked: false,enable: false,books: [] }


componentDidMount =()=>{
	ifAbonner(this.props.match.params.author).then(response => {
if(response.data === 'yes'){
  this.setState({
	...
	authorsBooks(this.props.match.params.author).then(response => {
	  this.setState({
      books: response.data
	  });
  });
...
const new_Abonnement = () => {
  axios.get('http://127.0.0.1:5001/book/abonnement/new', {params : {_email:  localStorage.getItem('email'),  _author: this.props.match.params.author}})
...
const delete_Abonnement = () => {
    axios.get('http://127.0.0.1:5001/book/abonnement/delete', {params : {_email:  localStorage.getItem('email'),  _author: this.props.match.params.author}})
    .then(function (response) {
      // setter
      //const token = localStorage.setItem('token', response.data.access_token)
      //const user = localStorage.setItem('user', response.data.user)
	...
	<div className="row no-gutters">
		...
		<div className="col-md-9">
			<div className="card-body">
				<h5 className="card-title">{ this.props.match.params.author }</h5>
